...
void Stack<T>::Push(const T& item) {
  if (num_items == max_items){
    size_t fresh = max_items * EXPANSIONFACTOR;
    Resize(fresh);
    max_items = fresh;
  }
...
T Stack<T>::Pop() {
  ...
  size_t shrink = max_items / EXPANSIONFACTOR;
  //std::cout << "shrink1:" << shrink;
  if (shrink < DEFAULTCAPACITY){
    shrink = DEFAULTCAPACITY;
  }

  if (num_items < (max_items / SHRINKRATE)){
    max_items = shrink;
    //std::cout << "shrink2:" << shrink;
    //std::cout << "size:" << num_items;
    Resize(shrink);
  }
  return item;
};
...
T Stack<T>::Peek() {
  ...
  return items[num_items-1]; // REPLACE THESE LINES
};
